...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-background font-sans text-text selection:bg-primary selection:text-background relative md:cursor-none cursor-auto">
      <CustomCursor />
      ...
      <header 
        className={`fixed w-full z-50 transition-all duration-300 ${
          scrolled || isMenuOpen ? 'bg-background/95 backdrop-blur-md shadow-lg shadow-primary/5 py-3' : 'bg-transparent py-5'
        }`}
      >
        <div className="container mx-auto px-4 flex justify-between items-center">
          ...
          <nav className="hidden md:flex items-center space-x-8">
            ...
            <button
              ...
              className="w-9 h-9 rounded-full flex items-center justify-center text-muted hover:text-primary border border-foreground/10 hover:border-primary/40 hover:bg-foreground/5 transition-colors"
            >
          ...
          <div className="md:hidden flex items-center gap-2">
            <button
              onClick={toggleTheme}
              aria-label={`Switch to ${theme === 'dark' ? 'light' : 'dark'} mode`}
              className="w-9 h-9 rounded-full flex items-center justify-center text-muted hover:text-primary border border-foreground/10 transition-colors"
            >
              <FontAwesomeIcon icon={theme === 'dark' ? faSun : faMoon} className="text-sm" />
            </button>
            <button
        ...
        <AnimatePresence>
          {isMenuOpen && (
            <motion.div
              ...
              className="md:hidden bg-background/95 border-t border-foreground/10 overflow-hidden backdrop-blur-md"
            >
              <div className="px-4 py-4 space-y-2">
                {navLinks.map((link) => (
                  <a 
                    key={link.name}
                    href={link.href} 
                    className="block px-3 py-2 rounded-md text-base font-medium text-muted hover:text-primary hover:bg-foreground/5" 
                    onClick={closeMenu}
                  >
                    {link.name}
                  </a>
      ...
      <footer className="bg-dark text-muted py-12 border-t border-foreground/5">
